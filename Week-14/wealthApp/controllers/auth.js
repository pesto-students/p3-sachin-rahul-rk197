...
const register = async (req, res) => {
    ...
    User.findOne({ email: newuser.email }, function (err, user) {
        ...
        if (!req.body.password) return res.status(400).json({ success: false, message: "Password is Mandatory" });
        ...
        newuser.save((err, doc) => {
            ...
            sendConfirmationEmail(newuser.firstName, newuser.email, newuser.confirmationToken, "email-verification");
        });
    });
};
    ...
    User.findByToken(token, (err, user) => {
        ...
        if (user)
            ...
            User.findOne({ email: req.body.email }, function (err, user) {
                if (!user) return res.json({ isAuth: false, message: " User With This Email Don't Exist" });
